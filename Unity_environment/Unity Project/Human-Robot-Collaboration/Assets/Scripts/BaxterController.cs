...
public class BaxterController : MonoBehaviour
{
    ...
    public void Init(GameObject baxter)
    {
        ...
        string gripper_base_L = hand + "/" + side + "_gripper_base/left_gripper";

        gripperBaseL = baxter.transform.Find(gripper_base_L);
        ...
        string gripper_base_R = hand + "/" + side + "_gripper_base/right_gripper";

        gripperBaseR = baxter.transform.Find(gripper_base_R);
    ...
    public void OpenCloseRight(Bool response)
    {
        if (response.data == true)
        {
            Debug.Log("Right gripper closed.");
            CloseGripper("right");
        }
        else
        {
    ...
    public void OpenCloseLeft(Bool response)
    {
        if (response.data == true)
        {
            Debug.Log("Left gripper closed.");
            CloseGripper("left");
        }
        else
        {
    ...
    public PoseStamped GetLeftGripper()
    {
        ...
        var header = new Header(seq: (uint)1, stamp: new Time((uint)now.Second, 0), frame_id: "left_gripper");
        var leftGripperPose= new PoseStamped
        {
            header = header,
        ...
        return leftGripperPose;
    }
    public PoseStamped GetRightGripper()
    {
        var now = DateTime.Now;
        var header = new Header(seq: (uint)1, stamp: new Time((uint)now.Second, 0), frame_id: "right_gripper");
        var rightGripperPose = new PoseStamped
        {
            header = header,
            pose = new RosMessageTypes.Geometry.Pose
            {
